 ...
 * never unmounts.
 *
 * The last scenario is the counter-example, and the reason the bound is derived
 * rather than picked: it overrides the switcher's max-width with a fixed `42vw`
 * and shows the overlay disappearing at the same viewport, with the same crews.
 *
...
/**
 * 1280px wide on purpose: it is both an ordinary laptop width and the width at
 * which a fixed `42vw` cap costs the search overlay entirely, so one viewport
 * carries both the fix and the counter-example.
 */
...
async function main() {
   ...
   * @param opts.capOverride  CSS max-width forced onto the switcher, for the
   *                          fixed-fraction counter-example
   */
  async function scenario(name, pinIds, opts = {}) {
    ...
    if (opts.capOverride) {
      await page.evaluate(cap => {
        ...
        if (wrapper instanceof HTMLElement) wrapper.style.maxWidth = cap
      }, opts.capOverride)
    }
  ...
  // 5. THE COUNTER-EXAMPLE. Same viewport, same crews, but capped at a fixed 42vw
  //    instead of the overlay-derived bound: the row reaches far enough that the
  //    search overlay unmounts entirely.
  await scenario('05-fixed-42vw-loses-search', ['devdesk', 'prod', 'staging', 'sandbox'], {
    capOverride: '42vw',
  })
  ...
  const counterExampleName = '05-fixed-42vw-loses-search'
  const derived = results.filter(r => r.name !== counterExampleName)
  const bad = derived.filter(r => !r.overlayPresent || r.clearsOverlay !== true)
  const counterExample = results.find(r => r.name === counterExampleName)
  ...
  if (counterExample && counterExample.overlayPresent) {
    console.error(
      'FAIL: the 42vw counter-example no longer reproduces — either the actions '
      + 'cluster moved or the overlay geometry changed, so this evidence is stale.',
    )
    process.exit(1)
  }
  console.log('OK')
