...
describe('InstanceTabBar', () => {
  ...
  it('caps the inline switcher against the centered search rather than a fixed vw', async () => {
    // The bound is the search overlay's own geometry (see lib/topbarLayout): a
    // hardcoded vw fraction cannot track a viewport-relative overlay, and this
    // is what keeps a long pinned row from squeezing it off screen.
    setCrewPins(['cd-1'])
    vi.mocked(api.listInstances).mockResolvedValue(listResp([conn()]))
    const store = createTestStore({
      instances: { warm: { 'cd-1': { port: 7778, token: 't' } }, activeId: null, mru: ['cd-1'], unread: {} },
    ...
    const row = await screen.findByTestId('crew-chip-row')
    const capped = row.closest('[style*="max-width"]') as HTMLElement | null
    expect(capped).not.toBeNull()
    expect(capped!.style.maxWidth).toBe(crewSwitcherMaxWidth())
  })
})
