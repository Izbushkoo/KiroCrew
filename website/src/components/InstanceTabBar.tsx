...
import { crewSwitcherMaxWidth } from '../lib/topbarLayout'
...
// Persisted preference: which crews are PINNED out of the dropdown into
// always-visible chips beside it. A user who lives in two or three crews pins
// those, so switching between them costs no dropdown click, while the rest stay
// behind the dropdown and cost the header no width. Empty by default — the
// compact dropdown alone stays the norm.
//
// Pinning is per crew rather than one expand-everything switch because the
// header's budget is a PIXEL budget (crewSwitcherMaxWidth), not a crew count:
// three crews named after real hosts outgrow it while six short names fit.
// Letting the user choose WHICH crews are worth header space is what keeps that
// budget spendable on the ones they actually switch between.
//
// Backed by a module-level store (not a per-component hook) because several bars
// in THIS realm can be mounted at once and must flip together the instant a pin
...
// per-component hook would leave a hidden bar on its stale value until a
// remount. It does NOT reach a remote pane's embedded bar — that runs in a
// separate cross-origin iframe realm with its own localStorage — so an embedded
// bar carries its own per-pane pin set.
 ...
 *
 * Pure, and exported, because the module-level store below reads storage exactly
 * once at import — a test cannot re-trigger that, so the decision has to be
...
function readPinned(): Set<string> {
  try {
    ...
    // Land the migration so it runs once, not on every load.
    if (stored === null && legacy !== null) {
      localStorage.removeItem(LEGACY_EXPANDED_PREF_KEY)
      safeSetItem(PINNED_PREF_KEY, JSON.stringify(ids))
    }
    return new Set(ids)
  } catch {
...
/**
 * Replace the pin set and broadcast to every mounted bar. A fresh Set identity
 * per write is load-bearing: `useSyncExternalStore` compares snapshots by
 * reference, so mutating in place would not re-render.
 */
...
function useCrewPins(): [Set<string>, (id: string) => void] {
...
function SwitcherRow({
  ...
  // The pin state rides on the row's own hover/accessible name rather than on
  // the glyph, because a `title` on a non-interactive span inside a menu item is
  // not reliably surfaced. `noRoom` has to be sayable: a pinned crew with no
  // visible chip otherwise looks like the pin silently failed.
  ...
  return (
    ...
    >
      ...
      {/* Filled = pinned and on screen; dimmed = pinned but clipped. The word is
          in the row's title, so this is decoration for sighted scanning only. */}
      {pinned ? (
        <Pin
...
/**
 * The dropdown holding every destination, plus the section that chooses which
 * crews get a header chip.
 *
 * The trigger is a bare chevron: the crew on screen is named by its own leading
 * chip, so repeating it here would spend header width twice on one fact. What the
 * chevron does carry is the AGGREGATE unread for crews that are not on screen —
 * pinned-but-clipped ones included — because that count is otherwise invisible.
 */
function SwitcherMenu({
  ...
  // chip currently on screen. A pinned crew whose chip got clipped counts, since
  // its badge went with it.
  const offScreen = entries.reduce((sum, e) => {
  ...
  const label =
    offScreen > 0
      ? i18nT('components.instanceTabBar.switch_crew_unread', { n: offScreen })
      ...
      <DropdownMenuTrigger asChild>
        ...
        >
          ...
          {offScreen > 0 ? (
            ...
            // badge that took layout space would feed its own measurement.
            <span
              aria-hidden
        ...
        {entries.map(entry => {
          ...
          return (
            <DropdownMenuItem
              ...
              // Two handlers, deliberately: `onClick` is the plain DOM event and
              // is what actually toggles, while `onSelect` exists only to
            ...
            >
              ...
              {/* Named on the row it belongs to, so the reason a pin shows no chip
                  is readable at the moment the user re-pins it. */}
 ...
 *
 * The row is a single nowrap line with `overflow: hidden`, so chips past its
 * width are simply not there. Measuring is what makes that state SAYABLE in the
 * dropdown — without it, a pinned crew with no visible chip reads as a pin that
 * silently failed.
 *
 ...
 * and contribute nothing to the header's width. Nothing sized by this measurement
 * is inside the thing being measured.
 */
function useClippedChipIds(
  ...
  useEffect(() => {
    ...
    measure()
    // Observing the row covers both causes of a re-fit: the viewport changing
    // (the row's width is a vw-derived max-width) and the chip set changing.
 ...
 * "there is more" instead of as a rendering fault, and the dropdown sitting
 * immediately after it is where the rest is.
 */
function CrewChipRow({
  ...
  return (
    <div
      ...
      // chips' offsetParent so useClippedChipIds can compare their offsetLeft
      // against this row's own clientWidth.
 ...
 *
 * The dropdown trails the chips rather than leading them, so it stays adjacent to
 * the last one and reads as "and the rest" — see `CrewChipRow` for why that
 * placement forces a clipped row rather than a wrapped one.
 *
 * The inline (header) variant caps its total width against the centered search
 * overlay — see `crewSwitcherMaxWidth` for the bound and the invariant that the
 * overlay can neither narrow nor vanish because of it. The `strip` variant is its
 * own full-width bar with nothing centered over it, so it takes no cap.
 */
function Switcher({
  entries,
  activeId,
  ...
  const [pinned, togglePin] = useCrewPins()
  ...
  // copies of one name would spend the budget saying the same thing twice.
  const chips = useMemo(
  ...
  return (
    <div
      ...
      style={variant === 'inline' ? { maxWidth: crewSwitcherMaxWidth() } : undefined}
    >
