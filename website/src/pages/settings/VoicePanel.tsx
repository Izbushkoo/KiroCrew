        ...
        <SettingsCard>
          {voiceQ.isError ? (
            ...
            <>
              ...
              <SettingsSelect label={i18nT('pages.settings.voicePanel.provider')} description={i18nT('pages.settings.voicePanel.piper_runs_locally_and_offline_polly_uses_aws_cr')} value={voiceCfg.provider} options={PROVIDER_OPTIONS} optionLabels={PROVIDER_OPTIONS.map(p => i18nT(PROVIDER_LABEL_KEY[p]))} onChange={v => { const patch: Partial<VoiceConfig> = { provider: v }; if (v === 'openai') { if (!['alloy', 'echo', 'fable', 'onyx', 'nova', 'shimmer'].includes(voiceCfg.voice)) patch.voice = 'alloy'; if (!['tts-1', 'tts-1-hd'].includes(voiceCfg.engine)) patch.engine = 'tts-1'; } setVoice(patch) }} disabled={voiceDisabled} />
              {isPolly ? (
                <>
                  <AwsConsentGate
                    service="polly"
                ...
                <>
                  <SettingsSelect label={i18nT('pages.settings.voicePanel.voice')} description="OpenAI TTS voice" value={['alloy', 'echo', 'fable', 'onyx', 'nova', 'shimmer'].includes(voiceCfg.voice) ? voiceCfg.voice : 'alloy'} options={['alloy', 'echo', 'fable', 'onyx', 'nova', 'shimmer']} optionLabels={['alloy (Стандартный нейтральный)', 'echo (Мужской тёплый)', 'fable (Выразительный британский)', 'onyx (Глубокий мужской)', 'nova (Энергичный женский)', 'shimmer (Тёплый женский)']} onChange={v => setVoice({ voice: v })} disabled={voiceDisabled} />
                  <SettingsSelect label="Model" description="OpenAI TTS model" value={['tts-1', 'tts-1-hd'].includes(voiceCfg.engine) ? voiceCfg.engine : 'tts-1'} options={['tts-1', 'tts-1-hd']} optionLabels={['tts-1 (Стандартная быстрая)', 'tts-1-hd (Высокое качество HD)']} onChange={v => setVoice({ engine: v })} disabled={voiceDisabled} />
