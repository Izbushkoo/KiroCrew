...
_WATCHER_SRC = (
    ...
    / "src" / "kiro_crew" / "knowledge" / "folder_watcher.py"
)
...
def test_ingest_file_issues_no_sqlite_call_on_the_event_loop():
    ...
    offenders = sorted(
        ...
        for called, lineno in _direct_calls(fn.body) if called in _DB_CALLS
    )
    assert offenders == [], (
...
def test_ingest_file_takes_its_item_ids_from_the_commit_callback():
    ...
    assert "SELECT id FROM items" not in body, (
...
async def test_ingest_file_reports_the_committed_ids_without_touching_sqlite(tmp_path):
    ...
    try:
        ...
        class _Pipeline:
            ...
            async def ingest_file(self, path, *, source_id, namespace,
                                  original_name, old_item_ids,
                                  on_committed=None, on_duplicate=None):
                def _insert_and_report() -> None:
                    ...
                    created.append(store.add_item(
                        "T", "body", "document", source_id=source_id))
...
@pytest.mark.asyncio
async def test_ingest_file_lets_cancellation_through_without_a_failed_state_row(
        tmp_path):
    ...
    try:
        ...
        task = asyncio.ensure_future(watcher._ingest_file(
            str(doc), source_id, "default", {}, []))
        await started.wait()
        task.cancel()
        ...
        row = store.db.execute(
            ...
            "AND file_path = ?", (source_id, str(doc))).fetchone()
...
async def test_scan_records_the_committed_group_on_the_state_row(tmp_path):
    ...
    try:
        ...
        owned = sorted(r["id"] for r in store.db.execute(
...
def test_await_ratchet_would_catch_a_reintroduced_post_commit_hop():
    ...
    assert names.count("to_thread") == 2, (
        f"the detector missed the post-commit hops (saw {names})")
...
def test_db_ratchet_would_catch_a_reintroduced_on_loop_query():
    ...
    assert {"execute", "fetchall"} <= hits, (
        f"the detector missed part of the pre-fix shape (found {sorted(hits)})")
