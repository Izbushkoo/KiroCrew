...
def _wait_for(predicate, timeout=5.0):
    """Poll until predicate holds. Used only by non-consent tests where the
    background work is a trivial no-op that completes in microseconds (e.g. a
    fake reader shutdown call). Consent-recheck tests use event-based
    synchronization instead — see ``_worker_event``."""
...
def _worker_event(monkeypatch):
    ...
    Also clears ``_check_in_flight`` if a stale worker from a prior test left it
    set — ``reset_for_testing`` does not wait for in-flight workers, so this flag
    can leak across tests.
    """
    import kiro_crew.metrics.provider as provider_mod

    # Ensure clean state: a prior test's daemon worker might not have reached its
    # finally yet when the test harness tears down its monkeypatch.
    with provider_mod._lock:
        provider_mod._check_in_flight = False
