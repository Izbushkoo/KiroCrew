...
def _openai_availability(stt_config) -> stt.Availability:  # type: ignore[no-untyped-def]
    key = _get_openai_api_key(stt_config)
    if key:
        return stt.Availability(True)
    return stt.Availability(False, stt.CODE_EXTRA_MISSING, "OpenAI API key missing")
...
def availability_detail(stt_config=None) -> stt.Availability:  # type: ignore[no-untyped-def]
    ...
    if provider == "openai":
        return _openai_availability(stt_config)
