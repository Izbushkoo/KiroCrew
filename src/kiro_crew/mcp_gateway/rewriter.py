...
def _resolve_target_command(
    ...
    # The spec's own env.PATH is the operator's explicit intent, so it is
    # probed FIRST; the augmentation applies only to the host PATH behind it —
    # augmenting the joined string would let a well-known dir shadow a
    # same-named binary the spec deliberately points elsewhere.
    search_path = os.pathsep.join(
        filter(None, [env_path, augmented_path(os.environ.get("PATH", ""))])
    )
