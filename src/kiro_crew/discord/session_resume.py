...
class DiscordSessionResume:
    ...
    async def leave_resumed_session(
        ...
        async with self._bind_lock:
            ...
            releasing = seen.key is not None or (sweep_ambiguous and seen.ambiguous)
            cleared: list[str] = []
            if releasing:
            ...
            try:
                ...
                unowned = self.resolve_inbound(channel_id)
                if (
                    expected is not None and not expected.retired
                    and unowned.key is None and not unowned.ambiguous
                ):
                    await self._expectations.retire_if(channel_id, expected.version)
            except ExpectationStoreError:
                logger.warning(
                    "discord resume: could not retire the record at channel %s while "
    ...
    async def route(self, channel_id: str) -> RoutingDecision:
        ...
        it restarts, an exhausted budget refuses. The state table lives in the spec
        section named above, which is the authority."""
        for _ in range(_MAX_ROUTE_ATTEMPTS):
            ...
            if (
                expected is not None and not expected.retired
                and expected.key == resolution.key
            ):
    ...
    async def settle(self, channel_id: str, decision: RoutingDecision) -> None:
        ...
        try:
            if decision.settle == _SETTLE_CLEAR:
                await self._expectations.retire_if(
